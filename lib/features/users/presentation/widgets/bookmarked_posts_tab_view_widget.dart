...
class _BookmarkedPostsTabViewPageState<C extends FeedsCubit> extends State<BookmarkedPostsTabViewPage> with AutomaticKeepAliveClientMixin {
  ...
  bool userBookmarkedPostsInitialized = false;
  ...
  void establishConnection() async {
    ...
    cubeChatConnectionStateStream = Connectivity().onConnectivityChanged.listen((connectivityType) async {
      if(await isNetworkConnected() && mounted) {
        if(!userBookmarkedPostsInitialized && pagingController != null) {
  ...
  Future<(String?, List<FeedModel>?)> fetchData(int pageKey) async {
    ...
    final result = await feedsCubit.fetchFeeds(path: path, pageKey: pageKey, queryParams: {"limit": AppConstants.gridPageSize, "page": pageKey});
    if(pageKey == 1 && result.$2 != null) {
      userBookmarkedPostsInitialized = true;
    }
    return result;
  }
