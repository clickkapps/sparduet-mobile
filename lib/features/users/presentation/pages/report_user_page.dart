...
class ReportUserPage extends StatefulWidget {
  ...
  final String? reason;
  const ReportUserPage({super.key, this.controller, this.reason, required this.user});
...
class _ReportUserPageState extends State<ReportUserPage> {
  ...
  void submitReportHandler(BuildContext context) {
    ...
    if(!validated) {
      context.showSnackBar("Kindly state the reason for reporting this user", appearance: NotificationAppearance.info);
      return;
    }
    ...
    context.read<UserCubit>().reportUser(offenderId: widget.user.id, reason: reason);

  }

  void userCubitListener(BuildContext context, UserState event) {
    if(event.status == UserStatus.reportUserSuccessful) {
      if(context.mounted) {
        context.showSnackBar("Thanks for reporting this user");
        context.popScreen();
      }
    }

    if(event.status == UserStatus.reportUserFailed) {
      if(context.mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text("Report user", style: theme.textTheme.bodyLarge?.copyWith(fontSize: 16, fontWeight: FontWeight.bold),),
        ),
